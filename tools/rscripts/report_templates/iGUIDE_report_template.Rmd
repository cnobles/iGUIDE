...
if( args$tables ){
  ...
  write.csv(
    x = spec_overview, 
    file = file.path(tables_path, "tbl1.specimen.overview.csv"),
...
if( args$tables ){
  ...
  target_tbl %>%
    ...
    write.csv(
      file = file.path(tables_path, "tbl2.target.sequences.csv"),
      quote = TRUE, row.names = FALSE
    )
...
if( args$tables ){
  ...
  write.csv(
    x = ot_tbl_save, 
    file = file.path(tables_path, "tbl3.ontarget.summary.csv"),
    quote = TRUE, row.names = FALSE
  )
...
kable(
    ...
    escape = FALSE, caption = tbl_caps[3], align = c("l", "l", rep("r", 4))
  ) %>%
...
if( args$tables ){
  ...
  write.csv(
    x = ft_tbl_save, 
    file = file.path(tables_path, "tbl4.offtarget.summary.csv"),
    quote = TRUE, row.names = FALSE
  )
}
...
kable(
    ...
    format.args = list(big.mark = ","), booktabs = TRUE, longtable = TRUE,
    escape = FALSE, caption = tbl_caps[4]
  ) %>%
  kableExtra::add_header_above(
    c(" " = 2, "All", "Pileup", "Paired", "Matched")
  ) %>%
...
if( args$tables ){
  ...
  write.csv(
    x = enrich_save, 
    file = file.path(tables_path, "tbl5.gene.enrichment.csv"),
    quote = TRUE, row.names = FALSE
  )
...
for( i in seq_along(ft_seqs_list) ){
  ...
  if( report_format == "html" ) cap <- paste0("Table ", i+5, ". ", cap)
