...
tbl_caps <- c(
  ...
  "Percent On-target.",
  "Estimate of On-target editing efficiency (percent) for each target by specimen.",
  "Off-target Loci.",
...
if( args$format == "html" ){
  tbl_caps <- paste0("Table ", 1:7, ". ", tbl_caps)
}
...
Incorporation sites, or locations in the genome where the dsODN was detected, are expected to be in the proximity of nuclease targeted locations. The target sequences provided for these analyses and their On-target locations (`Edit Locus`) are shown in **Table 2**. The genomic locations are in a format where chromosome, orientation, and nucleotide position are delimited by a colon (":").  
...
**Table 3** displays the percent of observations that were associated with On-target loci for **All** alignments. Further the percentages for **Pileups**, **Paired**, and **Matched** criteria are displayed in the following columns. 
...
if( args$tables ){
  ...
  names(ot_tbl_save) <- c(
    "Specimen", "Condition", "All percent", "Pileup percent", 
...
kable(
    ...
    escape = FALSE, caption = tbl_caps[4], align = c("l", "l", rep("r", 4))
  ) %>%
  kableExtra::add_header_above(
    c(" " = 2, "All", "Pileup", "Paired", "Matched")
  ) %>%
  kableExtra::kable_styling(
    bootstrap_options = bootstrap_options,
    latex_options = latex_options
...
**Table 5** below displays the estimated On-target editing efficiency based on the iGUIDE analysis. These values are calculated by the ratio of On-target editing to the sum of editing associated with the specific target method. This efficiency percentage is analogous to the specificity for each target. This is an estimate though, as On-target editing does have the potential to saturate the dynamic range of the abundance calculation. Therefore, these percentages should be considered lower bounds for editing efficiency and specificity.
...
ot_eff_data <- ot_eff_summary %>%
  ...
  dplyr::mutate_all(pNums, na.replace = "\\-", digits = 3)
...
kable(
    ...
    booktabs = TRUE, longtable = TRUE, escape = TRUE, caption = tbl_caps[5], 
    align = c("l", "l", rep("r", ncol(ot_eff_summary)-2))
  ) %>%
  kableExtra::kable_styling(
    bootstrap_options = bootstrap_options,
    latex_options = latex_options
...
if( args$tables ){
  ...
  names(ft_tbl_save) <- c(
    "Specimen", "Condition", "All loci", "Pileup loci", "Paired loci", 
    "Matched loci"
  )
  
  write.csv(
    x = ft_tbl_save, 
    file = file.path(tables_path, "tbl6.offtarget.summary.csv"),
    quote = TRUE, row.names = FALSE
  )
}
...
Flanking **Paired** loci and Target **Matched** loci are tested for enrichment against specific gene lists in **Table 5**. The cancer-associated and special gene lists included in this analysis were: `r paste(unique(c(sapply(configs, function(x) x$oncoGeneList$file), sapply(configs, function(x) x$specialGeneList$file))), collapse = " and ")`. Enrichment was tested by Fisher's Exact and p-values were adjusted for multiple comparisons using a Benjamani-Hochberg correction. Omitted specimens or conditions had insufficient data for this analysis (Total Gene Count = 0) or did not have enough data to support a powerful analysis (Estimated Power <= 0.75).
...
if( args$tables ){
  ...
  write.csv(
    x = enrich_save, 
    file = file.path(tables_path, "tbl7.gene.enrichment.csv"),
    quote = TRUE, row.names = FALSE
  )
...
enrich_df <- enrich_df %>%
  dplyr::filter(onco.power >= 0.75 | special.power >= 0.75) %>%
  dplyr::mutate(
  ...
  select(-onco.is.sig, -special.is.sig, -onco.power, -special.power)

names(enrich_df) <- c(
  "Origin", "Condition", "genes", "genes", "p-value (pwr)", 
  "genes", "p-value (pwr)"
)
...
if( nrow(enrich_df) > 0 ){
  ...
  kable(
      x = enrich_df, format = report_format, digits = 4, row.names = FALSE,
      format.args = list(big.mark = ","), booktabs = TRUE, longtable = TRUE,
...
full_target_seqs <- structure(
  sapply(seq_len(nrow(target_tbl)), function(i){
    ...
    target_tbl[i,] %$%
    ifelse(
      nuc_profiles[[nuclease]]$PAM_Loc == "3p",
      paste0(sequence, nuc_profiles[[nuclease]]$PAM),
      ifelse(
        nuc_profiles[[nuclease]]$PAM_Loc == "5p",
        paste0(nuc_profiles[[nuclease]]$PAM, sequence),
        sequence
      )
    )
  
  }),
  names = target_tbl$target
)
...
for( i in seq_along(ft_seqs_list) ){
  ...
  if( report_format == "html" ) cap <- paste0("Table ", i+7, ". ", cap)
  ...
  df <- dplyr::select(
    ...
    dplyr::mutate(
      aligned.sequence = divSeq(
        ...
        ref = unique(aligned.sequence[target == "On"])
      ),
