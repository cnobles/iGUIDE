...
```{r incorp_dist, fig.cap=fig_caps[1], fig.width=7.5, fig.height=2.5*(ceiling(nrow(sites_included) * nrow(gRNAs) / 2))}
...
```{r off-target_seqs, fig.cap=fig_caps[3], fig.width=8, fig.height=2.5*length(ot_seqs_list)}
ot_seq_plots <- lapply(ot_seqs_list, function(x){
...
gridExtra::grid.arrange(grobs = ot_seq_plots, ncol = 1)
```
