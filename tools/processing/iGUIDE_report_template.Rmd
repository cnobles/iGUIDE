...
```{r gRNA_tbl}
gRNAs$`Edit Locus` <- sapply(gRNAs$Guide, function(x){
...
pander(
  ...
  justify = "center", style = "multiline")
```
...
Flanking **Paired** loci and gRNA **Matched** loci are tested for enrichment against specific gene lists in Table 5. The cancer-associated and special gene lists (adjusted in the config file) included in this analysis were: `r paste(unique(c(sapply(configs, function(x) x$oncoGeneList$file), sapply(configs, function(x) x$specialGeneList$file))), collapse = " and ")`. Enrichment was tested by Fisher's Exact and p-values were adjusted for multiple comparisons using a Benjamani-Hochberg correction.
...
ft_seq_plots <- lapply(ft_seqs_list, function(x){
  ...
  select(y, aligned.sequence, mismatch, target, algns, gene_id) %>%
    seq_diverge_plot(
...
#      aspect.ratio = nrow(y)/nchar(ref_gRNA))
})

layout_mat <- matrix(
  c(
    rep(seq_along(ft_seqs_list), 
        sapply(ft_seqs_list, function(x) min(nrow(x), 10)) + 4)),
  ncol = 1)
